...
\title{EXTENDED RTABMAB AND REDUCED FAST SCNN FOR AUTONOMOUS INDUSTRIAL VEHICLE NAVIGATION}
...
\begin{IEEEkeywords}
	navigation system, RTAB-Map, Fast-SCNN, graph-based SLAM, machine learning, lane detection, autonomous vehicle 
\end{IEEEkeywords}
...
In Table~\ref{tab:pose_estimation}, \textbf{Lap-} represents the lap number, while \textbf{X (m)} and \textbf{Y (m)} denote the final position of the vehicle (in meters) after traveling 500~meters. The \textbf{Error (m)} column shows the deviation of the estimated pose from the ground truth, which was defined as $(x=0, y=0)$. The average error of 0.494~m (49.4~cm) indicates that hardware odometry alone produces significant drift over time, rendering it unsuitable for precise autonomous navigation. Therefore, pose estimation improvement using the Extended RTAB-Map is essential.
...
The same parameters are used for Table~\ref{tab:pose_estimation_slam}. After traveling 500~meters, the average positional error is 0.026~m (2.6~cm), demonstrating a significant improvement over the hardware odometry-only approach. This level of precision is sufficient for autonomous navigation in industrial environments, where centimeter-level accuracy is typically required.

\par
Another way to visualize the accuracy improvement is through occupancy grid maps. Figures~\ref{fig:map_hw_odom} and~\ref{fig:map_extended_rtabmap} compare the results between hardware odometry and the Extended RTAB-Map. In these maps, the blue line indicates the ground-truth trajectory.

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{../konten/map_salah.png}
	\caption{Occupancy grid map generated using hardware odometry only.}
...
Figures~\ref{fig:map_hw_odom} and~\ref{fig:map_extended_rtabmap} clearly illustrate the difference between the two methods. The map produced by hardware odometry deviates substantially from the ground truth, exhibiting distorted features due to drift accumulation. In contrast, the map generated by the Extended RTAB-Map aligns closely with the ground truth trajectory, confirming the effectiveness of the proposed approach in enhancing pose estimation accuracy.
...
In Table~\ref{tab:update_rate}, \textbf{RTAB-Map (Hz)} represents the average frequency of the default RTAB-Map, and \textbf{Extended RTAB-Map (Hz)} represents the frequency achieved by the proposed system. The average update rate increased from 1.123~Hz to 50.006~Hz, a nearly fiftyfold improvement. This demonstrates that integrating hardware odometry through the proposed skip connection and Kalman fusion significantly enhances temporal resolution, making real-time pose updates possible without requiring GPU acceleration.
...
\section{Conclusion} 
By utilizing and extending RTAB-Map as the primary algorithm for vehicle pose estimation, we achieve centimeter-level accuracy (error $<5$~cm). This is enabled by introducing a skip connection from hardware odometry to the pose fusion stage. During fusion, a Kalman Filter combines the corrected pose from RTAB-Map with the odometry-based state, yielding smooth and accurate estimates. The final fused pose attains a 50~Hz update rate on an Intel i5 12th Gen system with 16~GB RAM and no dedicated GPU.
...
\par
By combining the Extended RTAB-Map and the Reduced Fast-SCNN, we realize a CPU-only navigation system that delivers high-accuracy, high-frequency pose estimation together with real-time lane detection—well suited for industrial autonomous vehicles operating under compute and latency constraints.
