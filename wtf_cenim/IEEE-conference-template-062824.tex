...
% \begin{abstract}
% 	Indonesia Smelting Technology (IST) is an industry below Toyota Tsusho Corp that focuses on producing aluminum liquid. The melted aluminum liquid will be used by CMWI (Central Motor Wheel Indonesia) to make the wheel rims. IST has a vision to become a smart factory that utilizes the latest technology in its operations. One of the technologies that can be implemented is autonomous vehicles, which can transport a dangereous aluminum liquid (molten) from IST to CMW. The total weight of the molten aluminum liquid is around 1.8 tons, and the total weight that the vehicle need to carry is 4.8 tons. Previously, IST has used a Towing to transport molten to CMWI with manually drive by workres. The research is convert the manual Towing to fully autonomous Towing. The Autonomous Towing will be equipped with a navigation system that can detect the lane, and can navigate autonomously to the destination. The navigation system will use a combination of machine learning and graph-based SLAM (Simultaneous Localization and Mapping) to achieve this goal. The machine learning algorithm will be used to detect the lane using segmentation technique, while the graph-based SLAM will be used to estimate the pose of the Towing and create a map of the environment. The result of this research is a cm level error (below 1 cm) in robot pose estimation and a lightweight machine learning model that can run only on CPU with below 10 ms inference time.
% \end{abstract}

\begin{abstract}
	The research is convert the manual Towing to fully autonomous Towing for transport molten aluminum liquid in IST (Indonesia Smelting Technology). The Autonomous Towing will be equipped with a navigation system that can detect the lane, and can navigate autonomously to the destination. The navigation system will use a combination of machine learning and graph-based SLAM (Simultaneous Localization and Mapping) to achieve this goal. The machine learning algorithm will be used to detect the lane using segmentation technique, while the graph-based SLAM will be used to estimate the pose of the Towing and create a map of the environment. The result of this research is a cm level error (below 5 cm) in robot pose estimation and a lightweight machine learning model that can run only on CPU with below 10 ms inference time.
\end{abstract}
...
In this step, the image captured by the camera, originally a 3-channel image, is converted to 16 channels. This is achieved via three successive 3x3 convolution layers with a stride of 2: first converting 3→8 channels, then 8→16, and finally 16→32 channels. The resulting image is then split into two paths: one goes to global feature extraction, the other to feature fusion. 
...
There were several results that we got from this research. The first result is the cm level error (below 5 cm) in robot pose estimation. The second result is the fast lightweight and near accurate lane detection. The third result is the total time to transport the molten aluminum liquid from IST to CMWI. 
...
In the future, we plan to improve Graph Based SLAM system with shared memory for multi robot system. The database of the graph will be shared between robots so the all robot will have the same map of the environment. For offline processing, we can simply doing rsync to synchronize the database from one robot to another robot. So the all robot will use the same map of the environment. But for the online processing, we cannot use that because the database and map still being updated. To combine multi map from multi robot, we must have a transform for all maps. So for the online processing, we need a centralized system that can handle all of it. The challenge is how to make the centralized system can handle the large scale environment and can run realtime or semi-realtime. Another challenge is the communication between robots and centralized system. We need to make sure that the communication is reliable and can handle the large scale environment.
